...
export function generateAccountSetupPdf(res: Response) {
  ...
  res.setHeader("Content-Disposition", "attachment; filename=Greet-Account-Setup-Guide.pdf");
  doc.pipe(res);
  ...
  doc.moveDown(0.3);
  doc.fontSize(10).fillColor(GRAY).text("Pro services reference for onboarding new client accounts in Greet.");
  doc.moveDown(1);
  ...
  addStep(doc, 3, "Connect an Integration", [
    ...
    "  \u2022  Field mappings define how external data maps to Greet fields",
    "",
  ...
  addStep(doc, 4, "Configure Field Mappings", [
    ...
    "  \u2022  Target field \u2014 the Greet attendee field to map to",
