...
const logger = createChildLogger('ReplitAuth');
...
export async function setupAuth(app: Express) {
  ...
  app.get("/api/callback", (req, res, next) => {
    ...
    passport.authenticate(`replitauth:${req.hostname}`, (err: any, user: any, info: any) => {
      if (err) {
        logger.error({ err: err }, "Callback error");
        return res.redirect("/api/login");
      }
      if (!user) {
        logger.error({ err: info }, "No user returned from callback. Info");
        return res.redirect("/api/login");
      }
      req.logIn(user, (loginErr) => {
        if (loginErr) {
          logger.error({ err: loginErr }, "Login error");
          return res.redirect("/api/login");
        }
        logger.info("Successfully authenticated user:", user.claims?.email);
        return res.redirect("/");
      });
    })(req, res, next);
  });
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    req.session.save((err) => {
      if (err) {
        logger.error({ err: err }, "Failed to save session after token refresh");
      }
    });
    return next();
  } catch (error) {
